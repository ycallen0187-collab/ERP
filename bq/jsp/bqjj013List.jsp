<%@page import="com.icsc.ag.core.agjcB07Core"%>
...
<%! public static final String _AppId = "BQJJ013"; %>
...
public String action(dsjccom dsCom, HttpServletRequest request)throws Exception{ 
	...
	try {
		...
		Map bondYear= getBondYear(begDate,dsCom);
		String bonded_bDate = igfcuGetMapValue.getStringOrSpaceValue(bondYear,"BEGDATE");
	    ...
	    //System.out.println("bondeb_bDate = " + bondeb_bDate);
	    //System.out.println("bondeb_eDate = " + bondeb_eDate);
		
		long startTime = System.currentTimeMillis();
		 
		
		if(stockId.equals("B")){
			new agjcB07Core(dsCom, con).qryDatas_R(matNo, begDate, endDate, purpCode, stockId, bonded_bDate);
		}else if(stockId.equals("C")){
			new agjcbdT1PDAO(dsCom, con).qryDatas_R(matNo, begDate, endDate,purpCode,stockId ,bonded_bDate );
		}
...
try{
    ...
    if(purpCode.equals("Y")){
    	...
    	purpCode = " AND B.PURPCODE IN ('B','D','P') ";
    }
    
    
	 if(purpCode.equals("P")){
		purpCode = " AND B.PURPCODE IN ('P') ";
	}
    
    //累計部分利用子報表查詢  因為效能差寫在作業IIBAI
	...
	if(stockId.equals("C")){
		rptUrl.append(domain + "MP002/BQJJ001P");
	}else if(stockId.equals("B")){	
		rptUrl.append(domain + "MP002/BQJJ001PB");
	}
	
	rptUrl.append("&endDate=" + endDate);
	rptUrl.append("&bonded_bDate=" + bonded_bDate);
	rptUrl.append("&purpCode=" + purpCode);
	rptUrl.append("&stockId=" + stockId);
