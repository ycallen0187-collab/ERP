...
<%! public static final String _AppId = "BQJJ020"; %>
<%
	String sessionId = dejcWebUtil.genUniqueId(session.getId()) ;
	String formTarget = "bqjj020_"+sessionId ;
	String formAction = "/erp/bq/jsp/bqjj020List.jsp?_sessionId="+sessionId ;
%>
...
<de:form action="<%=formAction%>" target="<%=formTarget%>">
	<tr>
        ...
        <td width="15%" class="light-bg-left">
        	<de:text name="chkDispNo" size="30" value=" IN ( 'SCP15060086','SCP15060087' ) " required="true"/>
        </td>
        <td class="subsys-title" width="10%">成品料號</td>
        <td width="20%" class="light-bg-left">
        	<de:text name="BMATNO" size="25"   />
        </td> 
        
        <td class="subsys-title" width="10%">原料料號</td>
        <td width="20%" class="light-bg-left">
        	<de:text name="RAWMATNO" size="25"   />
        </td> 
        
        <td class="subsys-title" width="5%">報表</td>
        <td width="8%" class="light-bg-left">
			<de:select name="report_qry" src="de.Option" option="A=明細;G=彙總" hideValue="true"/>
		</td>
...
<div id="bqjj020Tab"  style="height:100%">
<iframe name="bqjj020_<%=sessionId%>" id="bqjj020Id" width="100%" height="90%" frameborder=0 src="/erp/bq/jsp/bqjj020List.jsp"></iframe>
</div>
</de:form>
